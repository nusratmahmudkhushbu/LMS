    ...
    return (
        <div className="books">
            <div className="container pt-4">
                <input type="text" placeholder="search for books" className="form-control text-center bg-light" value={search} onChange={handleSearchChange} />
                <hr />
                        ...
                        books.map(
                            book => (
                                <div className="col-12 col-md-2 m-1" key={book.book_id} style={{ backgroundColor: "#e5e5e5" }}>
                                    <Link to={`/book-details/${book.book_id}`}><img src={book.image_link} alt='Book' className="mt-4"/></Link>
                                    <p>{book.title}</p>
                                    <button className="btn btn-outline-success form-control mb-2" onClick={() => handleOnlineRead(book.book_id)}>Add To Bookshelf</button>
                                    <button className="btn btn-success form-control mb-2" onClick={()=>borrow(book.book_id)}>Request For Borrow</button>
                                </div>
                            )
                        )
                    }
